...
export default function SeriesPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await apiFetch('/recommendations?category=series&limit=40');
  ...
  return (
    <main className="min-h-screen bg-black p-6 text-white md:p-10">
      <section className="mb-10 rounded-[2rem] border border-white/10 bg-gradient-to-br from-[#00E0A8]/20 to-[#6A4CFF]/25 p-8">
        ...
        <p className="mt-4 max-w-3xl text-white/70">
          Seriale recomandate din date reale salvate în backend.
        </p>
      </section>
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-8 text-white/50">
          Nu există încă seriale salvate.
        </div>
      ) : (
